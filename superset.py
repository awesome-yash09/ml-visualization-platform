...
class DatabaseOperation:
    ...
    def login_self_hosted_superset(self, username, password):
        ...
        self.session.headers["X-Authorization"] = 'Bearer {}'.format(
        ...
        self.session.post("{}/login/".format(self.superset_url),
    ...
    def _refresh_token(self):
        r = requests.post(
            "{}/api/v1/security/refresh".format(self.superset_url), self.refresh_token)
    ...
    def _create_csrf_token(self):
        ...
        if token_request.status_code != 200:
            raise Exception(
                "Unable to get CSRF token: {}".format(token_request.text))
        return token_request.json().get('result')

    def create(self, metriql_url, database_name):
        raise Exception(
            "Operation is not  implemented. See: https://github.com/apache/superset/issues/16398")

        protocol = "https"
        uri = "trino://TODO:TODO@{}:443/metriql?protocol={}".format(
            metriql_url, protocol)
    ...
    def _build_metrics(measures, existing_metrics):
        ...
        for metric in existing_metrics:
            existing_metric_lookup[metric.get(
    ...
    def _build_columns(dimensions, measures, mappings, existing_columns):
        ...
        for metric in existing_columns:
            existing_column_lookup[metric.get(
        ...
        for name, (dimension, relation) in dimensions.items():
            ...
            columns.append(DatabaseOperation._build_column(
                column_id, name, dimension, True, mappings))
    ...
    def sync(self, database_id, metadata):
        ...
        if superset_dataset_requests.status_code == 401:
            ...
            raise Exception("Unable to perform operation: {}".format(
                superset_dataset_requests.text))
        ...
        for dataset in metriql_datasets:
            table_schema = "public" if dataset.get(
                'category') is None else dataset.get('category')
            ...
            existing_dataset_id = existing_dataset.get(
                'id') if existing_dataset is not None else None
            ...
            if existing_dataset is not None:
                superset_dataset_content = self.session.get(
                    "{}/api/v1/dataset/{}".format(self.superset_url,
                                                  existing_dataset_id),
                    params={
                        "columns": [
                            "columns", "metrics"
            ...
            metrics = DatabaseOperation._build_metrics(
                measures, existing_metrics)
            columns = DatabaseOperation._build_columns(
                dimensions, measures, mappings, existing_columns)
            ...
            if existing_dataset is None:
                ...
                if created_dataset_request.status_code not in [201, 200]:
                    raise Exception("Unable to create dataset: {}".format(
                        created_dataset_request.text))
            ...
            if updated_dataset_request.status_code != 200:
                raise Exception("Unable to update dataset: {}".format(
                    updated_dataset_request.text))
