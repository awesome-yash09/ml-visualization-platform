...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    dataset_id = None
    body = DatasetRestApiPost(
        database=2, schema=astradb_schema, table_name=table_name)
    try:
...
dashboard_created_response = client.session.post(
    url=base_url + "/dashboard", json=payload)
dashboard_created_id = dashboard_created_response.json()["id"]
print(dashboard_created_id)

response = create_chart_table(dashboard_created_id, dataset_id, "chart_testt", "table", [
                              "image_path", "class_labels", "classes", "confidence_score", "height", "image_view", "mode", "nchannels", "width"])
print(response)
